...
async function scraper(page, scrape, result, listChild, index) {
    for (let item in scrape) {
        if (scrape[item]['children']) {
            if (!scrape[item].list) {
                ...
                if (!result[scrape[item].name])
                    result[scrape[item].name] = [];
                let element = await scrapeElement(page, scrape[item].selector, scrape[item].type, scrape[item].list, false);
                console.dir(element.length);
                for (let b in element) {
                    result[scrape[item].name][b] = {};
                    result[scrape[item].name][b] = await scraper(page, scrape[item].children, result[scrape[item].name][b], scrape[item].list, b);
                }
            }
        }
        ...
        if (!scrape[item].list && !scrape[item]['children']) {
            result[scrape[item].name] = await scrapeElement(page, scrape[item].selector, scrape[item].type, scrape[item].list, index)
        }
        else if (!scrape[item]['children']) {
            result[scrape[item].name] = await scrapeElement(page, scrape[item].selector, scrape[item].type, scrape[item].list)
        }
    }
    return result;
}
async function scrapeElement(page, selector, type, list, index) {
    if (list || index) {
        const elements = await page.evaluate((selector, type) => {
            let custom = false;
        ...
        const element = await page.evaluate((selector, type) => {
            let custom = false;

            const link = document.querySelector(selector);
            if (type.includes('attr'))
                return link.getAttribute(type.split(":")[1]);
            switch (type) {
                case 'href':
                    return link.href
                case 'image':
